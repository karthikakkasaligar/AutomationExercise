...
public class TC_18 extends BaseTest {
	...
	public void View_Category_Products () {
	   ...
	   Assert.assertTrue(driver.findElements(By.id("categories are visible on left side bar")).isEmpty(),"categories are not visible");
	   
	   //to click on women category
	   homepage.Womencategory();
	   Assert.assertEquals(driver.findElement(By.cssSelector("[class='title text-center']")).getText(),"WOMEN - SAREE PRODUCTS");
	   homepage.Mencategory();
	   Assert.assertEquals(driver.findElement(By.cssSelector("[class='title text-center']")).getText(),"MEN - JEANS PRODUCTS");
